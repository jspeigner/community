"""
Lake Lanier · Tidbyt App
Layout 06 · Wave + Ticker
Live data from the Lanier Level Watch API (lanierlevel.com).

  Top-left   LANIER          (cyan)
  Top-right  water temp °F   (yellow)
  Center     hero diff + FT  (red if falling, green if rising)
  Middle     animated water surface, alert-zone-colored
  Bottom     scrolling ticker · alert-zone color bar
"""
...
# API refreshes hourly; cache 10 min to play nice with Tidbyt's app cycle.
CACHE_TTL = 600
...
WAVE_HEIGHT = 9  # rows of water (14-22)
BOTTOM_BAND_Y = 23  # first row of the divider/ticker/zone band

# ── Data fetch ───────────────────────────────────────────────────────
def fetch_latest():
    cached = cache.get("lanier_latest_v1")
    if cached:
    ...
    if not body or not body.get("data"):
        return None
    data = body["data"]
    cache.set("lanier_latest_v1", json.encode(data), ttl_seconds = CACHE_TTL)
    return data
...
def main():
    ...
    if not data:
        return render.Root(
            child = render.Box(
                color = COLOR_BLACK,
                child = render.Text("NO DATA", font = "tb-8", color = COLOR_RED),
            ),
        )
